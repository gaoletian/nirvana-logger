...
const errorStringify = err => {
  const res = Object.getOwnPropertyNames(err)
    .map(key => {
      return key === "stack"
        ? `${key}: ${err[key]}`
...
module.exports.default = module.exports = function(nameSpace = null) {
  const logger = function(...args) {
    ...
    try {
      ...
      const file =
        logger.fileName || (isk8OrProd ? filePath : color.red(filePath));
      const currentTime = isK8Env()
        ? formatDate("YYYY-MM-DD HH:mm:ss.SSS")
        : formatDate("YYMMDD/HHmmss.SSS");
      const timestamp = isk8OrProd ? currentTime : color.gray(currentTime);
      const scope = isk8OrProd
        ? "【" + nameSpace + "】"
        : nameSpace
        ? color.yellow("【" + nameSpace + "】")
        : "";
      ...
      return log(
        `${timestamp} ${file}${scope}${tractIdFormat}[${eatTime}]`,
        content
      );
    } catch (err) {
      console.trace(err);
